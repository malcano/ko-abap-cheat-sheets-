...
|---|---|---|---|---|---|
|`STANDARD`|Primary table index (Primary table index로 관리되기 때문에 [index table](https://help.sap.com/doc/abapdocu_cp_index_htm/CLOUD/en-US/index.htm?file=abenindex_table_glosry.htm)이라고 부름)|<ul><li>Table index</li><li>Table key</li></ul>|<ul><li>항상 중복 허옹(non-unique)하므로 중복 엔트리를 항상 허용</li><li>키가 상관이 없는 경우 empty key 정의 가능(`WITH EMPTY KEY`)</li></ul>|<ul><li>Table Index나 순차 처리를 통해 테이블 콘텐츠에 접근하는 경우 주로 사용.</li><li>Primary key 를 통한 Table 접근 응답 시간: 이러한 table access방식은 sorted table과 hashed table에서만 최적화 되어 있습니다. Standard Table에서의 Primary Key를 통한 접근은 모든 라인에 대해 선형 탐색하므로 라인 수가 많은(100줄 이상)standard table에서 사용하는 것은 권장되지 않습니다. </ul>|<ul><li>특정 정렬 순서가 없어도 `SORT` 구문을 사용하여 Table을 정렬할 수 있습니다. </li><li>Standard Table 테이블에서의 삽입: 테이블 끝 또는 특정 위치에 Line을 삽입할 수 있습니다.</li><li>[보조키(Secondary table keys)](https://help.sap.com/doc/abapdocu_cp_index_htm/CLOUD/en-US/index.htm?file=abensecondary_table_key_glosry.htm)를 정의하면 standard table에 보다 더 효율적으로 접근할 수 있습니다.</li><li>Standard Table, Sorted Table은 [관리 비용 (F1 docu for standard ABAP)](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/index.htm?file=abenadmin_costs_dyn_mem_obj_guidl.htm)이 가장 적습니다.</li></ul>|
|`SORTED`|Primary table index (Primary table index로 관리되기 때문에 [index table](https://help.sap.com/doc/abapdocu_cp_index_htm/CLOUD/en-US/index.htm?file=abenindex_table_glosry.htm)이라고 부름)|<ul><li>Table index</li><li>Table key</li></ul>|<ul>테이블 오름차순 정렬을 위한 Unique, Non-unique key</ul>|<ul><li>Table key와 index를 사용하여 테이블 콘텐츠에 대한 최적화된 접근이 가능한 경우.</li><li>Table Key를 통한 접근이 주요 접근 방법이지만, 고유 키(unique key)를 정의할 수 없는 경우</li></ul>|<ul><li>라인 삽입(INSERT) 및 삭제(DELETE) 시 자동으로 정렬이 수행됩니다.즉, 일반적으로 Table Index는 새롭게 구성됩니다.</li><li>이진탐색(Binary Search)가 사용되므로, Primary Key를 통해 테이블에 접근하는 경우 Table entry 에 따른 응답시간은 로그복잡도를 가집니다. (logarithmically)</li><li>Standard 테이블과 Sorted 테이블은 관리 비용이 가장 적습니다.</li></ul>|
|`HASHED`|해시 알고리즘 |<ul><li>Table key</li><li>[Secondary table index](https://help.sap.com/doc/abapdocu_cp_index_htm/CLOUD/en-US/index.htm?file=abensecondary_table_index_glosry.htm)</li></ul>|항상 Unique|<ul><li>무거운(Large) Internal Table 용</li><li>Key Access에 최적화됨. Table Key를 통한 접근이 주요 접근 방법이고, 고유 키(unique key)를 정의할 수 있음.</li></ul>|<ul><li>Primary Key를 통한 접근의 응답 시간은 일정하며, 테이블 엔트리 수와 무관.</li><li>관리 비용이 가장 높음.</li></ul>|
